...
(random downsampling + balanced kNN assignment).

Scaled pipeline using random downsampling.
"""
...
def run_datasail_scaled(dataset_name, downsample_ratio=0.25, k=5, balance_weight=2.0):
    ...
    print("\n5. Assigning remaining samples (balanced kNN)...")
    t0 = time.time()
    ...
    save_splits(train_df, val_df, test_df, dataset_name, "datasail_scaled_random")
    ...
    runtime_file = dataset_dir / "runtime_scaled_random.txt"
...
def main():
    ...
    print("\nAll scaled DataSAIL experiments complete")
